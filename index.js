...
process.argv.splice(env_loc, 2)

// from the selenium-cucumber-js instead of 'require('selenium-cucumber-js')'
...
cucumberCli.run(function (succeeded) {
  ...
  const lambdastatusStr = succeeded ? 'lambda-status=passed':'lambda-status=failed';
  console.log("Running lambdascript "+lambdastatusStr+" because "+succeeded);
  driver.executeScript(lambdastatusStr);
